      ...
      <div className="params-panel">
        ...
        <div className="params-panel__body">
          {meta.params.map(param => (
            <div className="param-row">
              <label className="param-label" id={`config-${param.name}`} style={{marginRight: 10}}>{param.name}</label>
              <ParamConfigurer
                param={param}
                onChange={val => {
        ...
        <div className="params-panel__footer">
          <div className="component-link-container" style={{marginRight: 10}}>
            <RelComponentLink index={index} components={components} dir="prev"/>
          </div>
          <div className="component-link-container" style={{marginLeft: "auto"}}>
            <RelComponentLink index={index} components={components} dir="next"/>
          </div>
        </div>
      </div>
    </div>
...
function RelComponentLink(props: {
  ...
  const target = dir === "prev" ? components[index === 0 ? components.length - 1 : index - 1] : components[index === components.length-1 ? 0 : index + 1];
  return <Link to={`/${target.meta.id}`}>{dir === "prev" ? "◄ " : ""}{target.meta.name}{dir === "next" ? " ►" : ""}</Link>;
}
  ...
  if (param.type === "string") {
    ...
    return (
      <Select
        ...
        {...commonProps}
      >
        {param.enums!.map(option => <Select.Option value={option} key={option}>{option}</Select.Option>)}
      </Select>
    );
